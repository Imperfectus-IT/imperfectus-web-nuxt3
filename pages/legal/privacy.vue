...
<template>
  <section class="mt-14 lg:mt-8">
    ...
    <div class="font-solina-extended-medium px-6 lg:px-[15%] xl:px-[30%]">
      <p class="mt-4">
        {{ $t("pages.legal.privacy.header.first_paragraph") }} <strong>{{ config.public.baseUrl }}</strong>
      </p>
      <div
      ...
      >
        <h2
          v-if="section?.title"
          class="font-medium text-[2rem] lg:text-[1.75rem]"
        >
          {{ $t(`pages.legal.privacy.${section.fragment}.title`) }}
        </h2>
        <p
          ...
          class="mt-4"
        >
          {{ $t(`pages.legal.privacy.${section.fragment}.paragraph_${p}`) }}
        </p>
        <ul
          v-if="section?.items"
          class="list-disc ml-8 mt-2"
        >
          <li
            ...
            class="py-1"
          >
            {{ $t(`pages.legal.privacy.${section.fragment}.items.item_${n}`) }}
          </li>
        </ul>
        <table
          v-if="section?.table_items"
          class="table-fixed mt-4"
        >
          <thead>
            <tr class="border-b border-green-tertiary/20">
              <th
                v-for="theadItem in section?.table_items?.thead"
                :key="theadItem"
              >
                {{ $t(`pages.legal.privacy.${section.fragment}.table_items.thead_${theadItem}`) }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="trItem in section?.table_items?.tr"
              :key="trItem"
              :class="{ 'border-b border-green-tertiary/20': trItem !== section?.table_items?.tr }"
            >
              <td
                v-for="tdItem in section?.table_items?.td"
                :key="tdItem"
                class="p-2 align-top"
              >
                {{ $t(`pages.legal.privacy.${section.fragment}.table_items.row_${trItem}_${tdItem}`) }}
              </td>
            </tr>
          </tbody>
        </table>
