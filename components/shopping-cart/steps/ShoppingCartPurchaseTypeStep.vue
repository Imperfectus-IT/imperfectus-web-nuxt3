...
<template>
  <div class="px-10 md:px-[28%] lg:px-[35%] 2xl:px-[40%]">
    ...
    <div v-if="isOrderPurchaseType" class="font-solina-extended-book text-base font-normal leading-6 p-6">
      <p class="mb-5">{{ $t("order.steps.stepPurchase.btn-order.section-1") }}</p>
    ...
    <div v-if="isSubscriptionPurchaseType" class="font-solina-extended-book text-base font-normal leading-6 py-6 px-2">
      <p class="mb-5">{{ $t("order.steps.stepPurchase.btn-subscription.section-1") }}</p>
      <p>{{ $t("order.steps.stepPurchase.btn-subscription.section-2") }}</p>
      <ul class="list-square ml-7">
        <li v-for="n in 3" :key="n"><MDC :value="$t(`order.steps.stepPurchase.btn-subscription.section-3.bullet-${n}`)"/></li>
      </ul>
      <div class="text-center mt-6">
        <Button
            severity="secondary"
